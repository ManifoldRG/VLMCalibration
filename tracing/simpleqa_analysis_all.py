...
from infini_gram_api_all import query_pretraining_api
from infini_gram_local_all import query_post_training_local

# Import utils
from utils import load_simpleqa_data, plot_confidence_vs_frequency_all, save_results, load_and_compare_zs_cot_data
...
def main():
    ...
    if args.zs_cot_compare:
        ...
        if scenario1_results:
            plot_confidence_vs_frequency_all(scenario1_results, args.output, 
                                        cot_correct_zs_incorrect=True)
        
        print(f"\nCreating plots for {len(scenario2_results)} CoT Incorrect/ZS Correct cases...")
        if scenario2_results:
            plot_confidence_vs_frequency_all(scenario2_results, args.output, 
                                        cot_incorrect_zs_correct=True)
    else:
        plot_confidence_vs_frequency_all(results, args.output, args.overconfident, args.underconfident)
