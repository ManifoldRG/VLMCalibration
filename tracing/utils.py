...
def plot_confidence_vs_frequency_all(raw_results: List[Dict], output_file: str, overconfident: bool = False, underconfident: bool = False, cot_correct_zs_incorrect: bool = False, cot_incorrect_zs_correct: bool = False):
    """Plot p_true vs document frequency and p_true vs n-gram count (pretraining + post-training) as separate plots with both linear and log scales."""
    if not raw_results:
        print("No data available for plotting")
        return
    ...
    filtered_results = []
    for r in raw_results:
        # if r["combined_freq"] > 0 or r["combined_ngram_count"] > 0:   # zero frequency points also included
        filtered_results.append(r)
